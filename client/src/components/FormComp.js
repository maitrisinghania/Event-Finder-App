...
export default class FormComp extends Component {
  ...
  events = async (latitude, longitude) => {
    ...
    console.log(`/events?keyword=${this.state.keyword}&category=${this.state.category}&longitude=${longitude}&latitude=${latitude}&radius=${this.state.distance}`)
    return await fetch(`/events?keyword=${this.state.keyword}&category=${this.state.category}&longitude=${longitude}&latitude=${latitude}&radius=${this.state.distance}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  keyword_auto = async () => {
    this.setState({loader: true},async () => {
      ...
      await fetch(`/suggest?keyword=${this.state.keyword}`)
        .then((res) => res.json())
        .then(data => {
